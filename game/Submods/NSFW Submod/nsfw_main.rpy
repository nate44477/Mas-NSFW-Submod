...
init python in mas_nsfw:
    ...
    def canShow_underwear():
        ...
        """
        if store.mas_getEVL("monika_gettingnude").shown_count >= 1 and store.mas_canShowRisque() and hour_check(set_hours=6) and not store.mas_SELisUnlocked(store.mas_clothes_underwear_white):
            return True
        else:
            return True
